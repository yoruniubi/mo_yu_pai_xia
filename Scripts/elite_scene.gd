...
func _ready():
	...
	# 1. 播放战斗音乐
	var battle_bgm = preload("res://Assets/Music/elite_bgm.mp3")
	...
	# 4. 初始化敌人数据 - 从精英敌人池随机选择
	var enemy = GameManager.get_random_elite_enemy()
	...
	if meta_for_slash and meta_for_slash.has_upgrade("sp_slash"):
		if GameManager.selected_hero and GameManager.selected_hero.card_pool.size() > 0:
			var slash_card = GameManager.selected_hero.card_pool[randi() % GameManager.selected_hero.card_pool.size()].duplicate()
			draw_pile.append(slash_card)
...
func apply_damage_to_hero(amount: int):
	...
	# 老油条鲶鱼：首次受伤减半
	if "鲶鱼" in enemy_name_label.text and not catfish_slipped_this_turn:
		final_damage = int(final_damage * 0.5)
		catfish_slipped_this_turn = true
		spawn_floating_number("滑溜！伤害减半", false, %BossSprite.global_position + Vector2(0, -140), Color.CYAN)
